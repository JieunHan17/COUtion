...
import '../model/donation_info.dart';
...
class DataSource {
  ...
  Future<List<DonationInfo>> getDonationInfoList() async {
    final response = await http.get(Uri.parse(
        'https://cfc32160-d826-4a35-afea-09537ed383b7.mock.pstmn.io/api/donation'));
    print('API response: ${response.body}');
    return jsonDecode(response.body)
        .map<DonationInfo>((json) => DonationInfo.fromJson(json))
        .toList();
  }
}